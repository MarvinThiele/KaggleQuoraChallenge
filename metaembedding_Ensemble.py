...
def load_and_prec():
    ...
    train_df, val_df = train_test_split(train_df, test_size=0.05, random_state=1338)
...
outputs = []
pred_val_y, pred_test_y = train_pred(model_gru_atten_3(embedding_matrix), epochs = 2)
...
pred_val_y, pred_test_y = train_pred(model_lstm_atten(embedding_matrix_1), epochs = 2) # Only GloVe
...
pred_val_y = np.sum([outputs[i][0]*coefs[i] for i in range(len(coefs))], axis = 0)
...
print(metrics.classification_report(val_y, pred_val_y, target_names=["Non-Toxic", "Toxic"]))
print(f"Combiend F1-Score: {metrics.f1_score(val_y, pred_val_y)}")